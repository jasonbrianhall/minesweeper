...
public:
  ...
  const std::vector<Score> &GetNativeHighscores() {
    return nativeMinesweeper->highscores
        .getScores(); // use nativeMinesweeper pointer
  }

  MinesweeperWrapper() { nativeMinesweeper = new Minesweeper(); }
  ...
  int GetAdjacentFlags(int row, int col) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
      for (int dx = -1; dx <= 1; dx++) {
        int newY = row + dy;
        int newX = col + dx;
        if (newY >= 0 && newY < nativeMinesweeper->height && newX >= 0 &&
            newX < nativeMinesweeper->width) {
  ...
  List<String ^> ^
      GetHighScores() {
        ...
        for (const auto &score : nativeScores) {
          String ^ scoreStr = String::Format(
              "{0,-20} {1,-10} {2,-10}", gcnew String(score.name.c_str()),
              (score.time / 60).ToString("D2") + ":" +
                  (score.time % 60).ToString("D2"),
  ...
  void ToggleFlag(int row, int col) {
    if (!nativeMinesweeper->revealed[row][col]) {
      nativeMinesweeper->flagged[row][col] =
          !nativeMinesweeper->flagged[row][col];
    }
  }
  ...
  bool IsGameOver() { return nativeMinesweeper->gameOver; }

  bool HasWon() { return nativeMinesweeper->won; }
  ...
  int GetWidth() { return nativeMinesweeper->width; }

  int GetHeight() { return nativeMinesweeper->height; }
  ...
  bool CheckWin() {
    ...
    for (int i = 0; i < nativeMinesweeper->height; i++) {
      for (int j = 0; j < nativeMinesweeper->width; j++) {
        ...
        if (!nativeMinesweeper->minefield[i][j] &&
            !nativeMinesweeper->revealed[i][j]) {
          return false;
        }
      }
    }
...
private:
  ...
  static const char *FLAG_BASE64 =
      R"(iVBORw0KGgoAAAANSUhEUgAAACAAAAAgBAMAAACBVGfHAAAABGdBTUEAALGPC/xhBQAAAAFzUkdCAdn
...
AGqwCSmixwhQaGhqEIgBUocSAEwAAjwQWTza+izoAAAAASUVORK5CYII=)";
  ...
  static const char *BOMB_BASE64 =
      R"(iVBORw0KGgoAAAANSUhEUgAAACAAAAAgBAMAAACBVGfHAAAABGdBTUEAALGPC/xhBQAAAAFzUkdCAd
...
LYx9Yppc2K6rnkZS3u1c8sXk6BRi54Lg1mbtV/gBxfI7i3nTTAoAAAAASUVORK5CYII=)";
  ...
  static const char *REVEALED_BASE64 =
      R"()";

  static const char *ICON_BASE64 =
      R"()";
  ...
  void InitializeComponent() {
    ...
    ToolStripMenuItem ^ setSeedMenuItem = gcnew ToolStripMenuItem(
        "&Set Seed", nullptr,
    ...
    timerBox->Location =
        Point(this->ClientSize.Width - 120, menuStrip->Height + 5);
    ...
    timerBox->Font = gcnew System::Drawing::Font(
        L"Consolas", 16,
        FontStyle::Bold); // Changed to Consolas as it's more commonly available
    ...
    flagCounterBox->Location = Point(this->ClientSize.Width - 120,
                                     menuStrip->Height + timerBox->Height + 10);
    ...
    flagCounterBox->Font =
        gcnew System::Drawing::Font(L"Consolas", 16, FontStyle::Bold);
    // This code was added as a cheat to check the highscores functionality
    // flagCounterBox->Click += gcnew EventHandler(this,
    // &MainForm::FlagCounterBox_Click);
  ...
  void ShowCustomGame_Click(Object ^ sender, EventArgs ^ e) {
  ...
  void MainForm_KeyDown(Object ^ sender, KeyEventArgs ^ e) {
    ...
    case Keys::F3:
      ...
      break;
    case Keys::F4:
        ShowCustomGame_Click(nullptr, nullptr);
        break;      
    case Keys::N:
  ...
  void ValueChanged(Object ^ sender, EventArgs ^ e) {
    ...
    CustomGameControls ^ controls =
        safe_cast<CustomGameControls ^>(changedControl->Tag);

    int maxMines =
        (int)(controls->widthInput->Value * controls->heightInput->Value) - 1;
  ...
  void ShowCustomGameDialog() {
    ...
    // Add event handlers
    widthInput->ValueChanged +=
        gcnew EventHandler(this, &MainForm::ValueChanged);
    heightInput->ValueChanged +=
        gcnew EventHandler(this, &MainForm::ValueChanged);
    ...
    // Add controls
    customForm->Controls->AddRange(
        gcnew array<Control ^>{widthLabel, widthInput, heightLabel, heightInput,
                               minesLabel, minesInput, okButton, cancelButton});
  ...
  void InitializeGrid() {
    ...
    gridPanel->Size =
        System::Drawing::Size(width * cellSize + 1, height * cellSize + 1);
    gridPanel->BackColor = Color::Gray;
    this->Controls->Add(gridPanel);

    grid = gcnew array < Button ^, 2 > (height, width);
    buttonFont = gcnew System::Drawing::Font(L"Lucida Console", cellSize / 3,
                                             FontStyle::Bold);
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        ...
        grid[i, j]->MouseUp +=
            gcnew MouseEventHandler(this, &MainForm::Cell_MouseUp);
  ...
  void ShowHowToPlay_Click(Object ^ sender, EventArgs ^ e) {
    MessageBox::Show(L"How to Play Minesweeper:\n\n"
                     ...
                     L"How to Play", MessageBoxButtons::OK,
  ...
  void ShowAbout_Click(Object ^ sender, EventArgs ^ e) {
    MessageBox::Show(L"Minesweeper\n\n"
                     L"A classic game where you must avoid the mines and clear "
                     L"the field.\n\n"
                     L"Left click: Reveal cell\n"
                     L"Right click: Flag/unflag cell\n"
                     L"Click on revealed number: Reveal adjacent cells if "
                     L"correct number of flags\n\n"
                     L"Written by Jason Hall (2025)",
                     L"About Minesweeper", MessageBoxButtons::OK,
                     MessageBoxIcon::Information);
  }
  ...
  void ShowHighScores() {
    ...
    highScoreForm->Anchor =
        static_cast<AnchorStyles>(AnchorStyles::Top | AnchorStyles::Bottom |
    ...
    for (const auto &score : nativeScores) {
      ...
      ListViewItem ^ item = gcnew ListViewItem(
          gcnew array<String ^>{gcnew String(score.name.c_str()), timeStr,
                                gcnew String(score.difficulty.c_str())});
    ...
    closeButton->Location =
        Point((buttonPanel->Width - closeButton->Width) / 2,
  ...
  void LoadBase64Images() {
    try {
      ...
      array<Byte> ^ flagBytes =
          Convert::FromBase64String(gcnew String(FLAG_BASE64));
      System::IO::MemoryStream ^ flagStream =
          gcnew System::IO::MemoryStream(flagBytes);
      flagImage = Image::FromStream(flagStream);

      // Convert base64 to image for bomb
      array<Byte> ^ bombBytes =
          Convert::FromBase64String(gcnew String(BOMB_BASE64));
      System::IO::MemoryStream ^ bombStream =
          gcnew System::IO::MemoryStream(bombBytes);
      bombImage = Image::FromStream(bombStream);

      // Convert base64 to image for revealed
      array<Byte> ^ revealedBytes =
          Convert::FromBase64String(gcnew String(REVEALED_BASE64));
      System::IO::MemoryStream ^ revealedStream =
          gcnew System::IO::MemoryStream(revealedBytes);
  ...
  void SetApplicationIcon() {
    try {
      array<Byte> ^ iconBytes =
          Convert::FromBase64String(gcnew String(ICON_BASE64));
      System::IO::MemoryStream ^ iconStream =
          gcnew System::IO::MemoryStream(iconBytes);
